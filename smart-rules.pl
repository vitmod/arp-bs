...
#echo '====> install $package ' &&
  ...
  {
    $output .= "UPDATEPKGV_$package = [ -d \$(DIR_$package) ] && (touch \$\@ -d `cd \$(DIR_$package) && \$(git_date) && cd -`) || true" . "\n";
...
sub process_prepare ($)
{
  ...
  my $updateversion = "";
    ...
    elsif ( $p eq "git" )
    {
      ...
      $updateversion = "[ -d $f ] && (true" . $output . "(touch \$(buildprefix)/\$\@ -d `cd $f && \$(git_date) && cd -`) ) || true";
      $output .= "cp -a " . $f . $subdir . " " . $dir;
